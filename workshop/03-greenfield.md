...
4. Under the "Event" pull down, change from "File Saved" to "Agent Stop". Ensure that the action is "Ask Kiro". The instructions for the Kiro agent will be different as these are generated by Kiro. This is what was produced when I created it - yours might be different, but that is ok.

```
A Kiro spec file has been modified. Please analyze the changes and append a summary entry to the changelog.md file. Include what was added, changed, or removed from the Kiro specifications. Format the entry with a timestamp and clear description of the changes.
```

5. Close the Agent hook configuration screen from the main editor panel. Your hook will now be automatically enabled, and we will test it by making a change to the Export spec.

6. From the Kiro screen, select the Export spec (it will be called something like todo-export or todo-csv-export). It should bring up the requirements.md file. From the Kiro chat interface, enter the following.
