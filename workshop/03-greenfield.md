...
4. Under the "Event" pull down, make sure it says "File Saved". This means that this agent hook will be triggered every time there is a file save event.

5. Under the "File Saved" you should see that under the file path to watch, it should have just three files: requirements.md, design.md, and tasks.md. If your agent hook screen does not have this, perhaps it might have a wildcard "*.*" change this and add each of these file separately using the dialog box.

6. Ensure that the action is "Ask Kiro". The instructions for the Kiro agent will be different as these are generated by Kiro. I modified the text that was generated to use this.
...
7. Close the Agent hook configuration screen from the main editor panel. Your hook will now be automatically enabled, and we will test it by making a change to the Export spec.
